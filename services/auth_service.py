...
from .face_service import capture_image, verify as face_verify
from utility import hash_passwd, verify as verify_password
from utility.passwordhide import get_hidden_input
from utility.sql_util import fetch_all, execute_query, commit_transaction, fetch_one
...
def login(username, password):
    ...
    if row is None:
        ...
        temp_passwd = row[0]
        if verify_password(password, temp_passwd):
...
def get_admin_display_name(admin_id):
    rows = fetch_all(
        f"Select first_name,middle_name,last_name from administrativestaff where admin_id = '{admin_id}'"
    )
    if not rows:
        return None
    f, m, l = rows[0][0], rows[0][1], rows[0][2]
